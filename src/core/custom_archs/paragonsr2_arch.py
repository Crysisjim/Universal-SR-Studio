...
try:
    ...
    # Running inside neosr venv (inference) — provide no-op registry so the file is importable.
    class _FakeReg:
        def register(self):
            return lambda c: c
...
@ARCH_REGISTRY.register()
def paragonsr2_realtime(scale=4, num_feat: int = 16, num_groups: int = 1,
                        num_blocks: int = 3, **kw):
    return ParagonSR2(
        scale=scale, num_feat=num_feat, num_groups=num_groups, num_blocks=num_blocks,
        variant="realtime",
...
@ARCH_REGISTRY.register()
def paragonsr2_stream(scale=4, num_feat: int = 32, num_groups: int = 2,
                      num_blocks: int = 3, **kw):
    return ParagonSR2(
        scale=scale, num_feat=num_feat, num_groups=num_groups, num_blocks=num_blocks,
        variant="stream",
...
@ARCH_REGISTRY.register()
def paragonsr2_photo(scale=4, num_feat: int = 64, num_groups: int = 4,
                     num_blocks: int = 4, **kw):
    return ParagonSR2(
        scale=scale, num_feat=num_feat, num_groups=num_groups, num_blocks=num_blocks,
        variant="photo",
...
@ARCH_REGISTRY.register()
def paragonsr2_ultimate(scale: int, num_feat: int = 180, num_groups: int = 9,
                        num_blocks: int = 8, **kw) -> ParagonSR2:
    """
    ParagonSR2 Ultimate "Intelligent Design" Flagship.
    Default: num_feat=180, num_groups=9, num_blocks=8 (~23M params)
    """
    return ParagonSR2(
        scale=scale, num_feat=num_feat, num_groups=num_groups, num_blocks=num_blocks,
        variant="ultimate",
        ...
        attention_mode=kw.pop("attention_mode", "sdpa"),
        export_safe=kw.pop("export_safe", False),
        window_size=kw.pop("window_size", 12),
        num_tokens=kw.pop("num_tokens", 32),
