...
try:
    ...
    class _FakeReg:
        ...
        def __call__(self, fn):
            return fn
    ARCH_REGISTRY = _FakeReg()  # type: ignore

try:
    from .resampler import MagicKernelSharp2021Upsample
except ImportError:
    # When injected into traiNNer/archs/, relative import fails on first run
    # until resampler.py is also present. Import by absolute path as fallback.
    import importlib.util as _ilu, os as _os
    _rsp = _os.path.join(_os.path.dirname(__file__), "resampler.py")
    if _os.path.exists(_rsp):
        _spec = _ilu.spec_from_file_location("resampler", _rsp)
        _mod = _ilu.module_from_spec(_spec)
        _spec.loader.exec_module(_mod)
        MagicKernelSharp2021Upsample = _mod.MagicKernelSharp2021Upsample
    else:
        MagicKernelSharp2021Upsample = None  # defer error to runtime
...
def paragonsr_anime(scale: int = 4, num_feat: int = 28, num_groups: int = 2,
                    num_blocks: int = 3, ffn_expansion: float = 1.5, **kwargs) -> ParagonSR:
    """
    ...
    - Default: num_feat=28, num_groups=2, num_blocks=3, ffn_expansion=1.5
    """
    return ParagonSR(
        scale=scale, num_feat=num_feat, num_groups=num_groups,
        num_blocks=num_blocks, ffn_expansion=ffn_expansion, **kwargs
    )


@ARCH_REGISTRY.register()
def paragonsr_nano(scale: int = 4, num_feat: int = 24, num_groups: int = 3,
                   num_blocks: int = 2, ffn_expansion: float = 1.5, **kwargs) -> ParagonSR:
    """
    ...
    - Default: num_feat=24, num_groups=3, num_blocks=2, ffn_expansion=1.5
    """
    return ParagonSR(
        scale=scale, num_feat=num_feat, num_groups=num_groups,
        num_blocks=num_blocks, ffn_expansion=ffn_expansion, **kwargs
...
@ARCH_REGISTRY.register()
def paragonsr_tiny(scale: int = 4, num_feat: int = 32, num_groups: int = 3,
                   num_blocks: int = 2, ffn_expansion: float = 2.0, **kwargs) -> ParagonSR:
    """
    ...
    - Default: num_feat=32, num_groups=3, num_blocks=2, ffn_expansion=2.0
    """
    return ParagonSR(
        scale=scale, num_feat=num_feat, num_groups=num_groups,
        num_blocks=num_blocks, ffn_expansion=ffn_expansion, **kwargs
...
@ARCH_REGISTRY.register()
def paragonsr_xs(scale: int = 4, num_feat: int = 48, num_groups: int = 4,
                 num_blocks: int = 4, ffn_expansion: float = 2.0, **kwargs) -> ParagonSR:
    """
    ...
    - Default: num_feat=48, num_groups=4, num_blocks=4, ffn_expansion=2.0
    """
    return ParagonSR(
        scale=scale, num_feat=num_feat, num_groups=num_groups,
        num_blocks=num_blocks, ffn_expansion=ffn_expansion, **kwargs
...
@ARCH_REGISTRY.register()
def paragonsr_s(scale: int = 4, num_feat: int = 64, num_groups: int = 6,
                num_blocks: int = 6, ffn_expansion: float = 2.0, **kwargs) -> ParagonSR:
    """
    ...
    - Default: num_feat=64, num_groups=6, num_blocks=6, ffn_expansion=2.0
    """
    return ParagonSR(
        scale=scale, num_feat=num_feat, num_groups=num_groups,
        num_blocks=num_blocks, ffn_expansion=ffn_expansion, **kwargs
...
@ARCH_REGISTRY.register()
def paragonsr_m(scale: int = 4, num_feat: int = 96, num_groups: int = 8,
                num_blocks: int = 8, ffn_expansion: float = 2.0, **kwargs) -> ParagonSR:
    """
    ...
    - Default: num_feat=96, num_groups=8, num_blocks=8, ffn_expansion=2.0
    """
    return ParagonSR(
        scale=scale, num_feat=num_feat, num_groups=num_groups,
        num_blocks=num_blocks, ffn_expansion=ffn_expansion, **kwargs
...
@ARCH_REGISTRY.register()
def paragonsr_l(scale: int = 4, num_feat: int = 128, num_groups: int = 10,
                num_blocks: int = 10, ffn_expansion: float = 2.0, **kwargs) -> ParagonSR:
    """
    ...
    - Default: num_feat=128, num_groups=10, num_blocks=10, ffn_expansion=2.0
    """
    return ParagonSR(
        scale=scale, num_feat=num_feat, num_groups=num_groups,
        num_blocks=num_blocks, ffn_expansion=ffn_expansion, **kwargs
...
@ARCH_REGISTRY.register()
def paragonsr_xl(scale: int = 4, num_feat: int = 160, num_groups: int = 12,
                 num_blocks: int = 12, ffn_expansion: float = 2.0, **kwargs) -> ParagonSR:
    """
    ...
    - Default: num_feat=160, num_groups=12, num_blocks=12, ffn_expansion=2.0
    """
    return ParagonSR(
        scale=scale, num_feat=num_feat, num_groups=num_groups,
        num_blocks=num_blocks, ffn_expansion=ffn_expansion, **kwargs
