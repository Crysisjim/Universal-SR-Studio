...
# Register with traiNNer so the arch is trainable (type: gfisrv2).
# Safe no-op for inference (direct import) if the registry is unavailable.
try:
    from traiNNer.utils.registry import ARCH_REGISTRY
    _REG = ARCH_REGISTRY.register()
except Exception:
    def _REG(c):
        return c
